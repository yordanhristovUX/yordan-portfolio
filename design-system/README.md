...
css/components.css      every component's styles (an ASSEMBLY: 18 blocks authored, 5 generated)
...
## The two gates over the stories

The build checks that a component *has* a story. These two check what the story *renders*.
Both drive `@storybook/test-runner` against a static Storybook that
...
**Neither joins the root `npm run check`,** and that is a contract rather than an oversight:
the root gate runs offline, with no browser and no heavy dependency, and these two need
Chromium (`npx playwright install --with-deps chromium`). They run in the path-filtered
...
two site headers. Both elements are `<div>` now. (`index.html` still ships the old pair — the
pages are not this directory's to edit.)
