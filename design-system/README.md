...
scripts/test-storybook.mjs  builds, serves and drives both  (npm run test:a11y / test:visual)
...
`scripts/test-storybook.mjs` builds when it is stale, serves on an ephemeral port, and tears
