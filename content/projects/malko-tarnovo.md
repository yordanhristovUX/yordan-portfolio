...
Two audiences with almost opposing needs. **Tourists** are explorers — visual richness, maps,
...
This is not decoration; it's a wayfinding system. A user glancing at a card knows instantly
whether it's a tourist recommendation or a municipal service — before reading a word. For
older users, colour becomes a cognitive shortcut.

## White-label design system {#system}

- **Semantic colour tokens** — swap the green/blue for any municipality's brand and it
  cascades through the entire system.
- **Spacing and rounding tokens** — no arbitrary values anywhere in the library.
- **Typography system** — Comfortaa, tokenised scale, friendly yet official.
- **Icon system** — municipal logo redesigned on a 64×80 grid, coherent from app icon to
  in-app header.

## Accessibility by default {#system}

- Touch targets exceeding WCAG's 44×44px minimum throughout.
- WCAG 2.1 AA contrast across both colour contexts.
- Hierarchy through size, weight, and spacing — never colour alone.
- Progressive disclosure for dense municipal content.
