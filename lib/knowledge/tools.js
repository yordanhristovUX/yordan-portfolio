...
function searchContent({ query, limit = SEARCH_LIMIT_DEFAULT }) {
  ...
  const results = search(content, query, n);
  return { query, limit: n, count: results.length, results, gated: false, gateMatched: gate.entity };
}
