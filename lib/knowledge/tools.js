...
async function searchContent({ query, limit = SEARCH_LIMIT_DEFAULT }) {
  /* The bound the schema cannot state, enforced where it holds — and the query
     is clamped FIRST, before the gate reads it, before the ranker embeds it and
     before it is echoed back in the result. Everything downstream sees `q`. */
  const q = String(query ?? "").slice(0, SEARCH_QUERY_MAX_CHARS);
  ...
  if (!gate) {
    return { query: q, limit: n, count: 0, results: [], gated: true, message: GATE_MISS_MESSAGE };
  }
