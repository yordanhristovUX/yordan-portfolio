...
const DESIGN_SYSTEM_RULES = [
  ...
  },
  {
    ...
    rule:
      "Dark mode and print are token VALUES, not media queries. Never write a `prefers-color-scheme` block, and never put a colour inside `@media print` — a themed colour is a token carrying a `dark` and/or `print` value, and the build emits the query and the `[data-theme]` override from it.",
    evidence: `${content.system.light} tokens have a light value, ${content.system.dark} a dark value and ${content.system.print} a print value — ${content.system.values} values from ${content.system.tokens} tokens.`,
  },
  {
...
function getDesignSystem() {
  return {
    ...
    counts: {
      ...
      themes: { light: content.system.light, dark: content.system.dark, print: content.system.print },
