 ...
 * nothing backs it.
 *
 * THE CALLER'S HALF IS NOT DONE. The documented policy is "retry once, then
 * degrade to an explicit 'not on file' block"; api/chat.js accepts the retry
 * whenever ANY block survives and never re-checks `hasUnsourcedClaim`, so the
 * shipped policy is "retry once, then ship whatever comes back" and an unsourced
 * claim that survives the retry reaches the browser. This module cannot fix
 * that from here — what it can do, and now does, is make `sources` a REQUIRED
 * top-level property of the respond schema so that the sources-free answer is
 * no longer schema-valid in the first place. Structural fix before retry
 * policy: the retry spends a whole model turn papering over a shape the schema
