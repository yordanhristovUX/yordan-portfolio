 ...
 *  should read.
 *
 *  A fourth set, `sole`, records the terms that EXHAUST one of this entity's
 *  authored names — see the note above it. It is scoped to the entity, never
 *  pooled, because "the whole of X's name" is a fact about X and about nothing
 *  else. */
export function buildNameSurfaces(content) {
  const surfaces = [];
  const add = (entity, nameParts, labelParts) => {
    const names = nameParts.filter(Boolean).map((p) => String(p).trim());
    const name = new Set(tokenize(names.join(" ")));
    const label = new Set(tokenize(labelParts.filter(Boolean).join(" ")));
    surfaces.push({ entity, name, label, sole: soleNames(names), terms: new Set([...name, ...label]) });
  };
...
/* ============================================================
   A name a single ordinary word EXHAUSTS

   The capitalisation test below reads the corpus's prose to decide whether a
   term is a name or ordinary vocabulary. It gets one class of real name wrong,
   and the class is not exotic: an organisation whose entire name IS an ordinary
   English word. `Independent` is the freelance role's org; the word also
   appears, lowercase and unrelated, inside Domestina's marketplace description
   ("independent professionals"), so the prose scan reads it as vocabulary and
   the role became unreachable by the only name it has.

   The separation is the same STRUCTURAL one the name/label split already makes,
   applied one level down. Compare the two cases the prose scan cannot tell
   apart:

       `audit` — one word of the id `greenstreet-audit`, and lowercase in the
                 prose that describes the audit. A query saying `audit` has
                 said one ordinary word of a longer name; a query NAMING that
                 entity would say Green Street. It must not open, and it is the
                 capitalisation test alone that stops it.
       `independent` — the whole of the org "Independent". There is no more of
                 the name to say. A query that says it has named the entity
                 exactly.

   So the exemption is: the term is, on its own, an entity's COMPLETE authored
   name, and the corpus capitalises that name as a proper noun. Both conditions
   are read off the content the author wrote — an org, a client, a title — never
   off the prose and never off a question set. Ids are lowercase slugs and so
   never qualify here; they do not need to, since a slug the prose never uses
   already passes the test below on df alone.

   THE SINGLE-WORD REQUIREMENT IS ON THE AUTHORED STRING, not on its token
   count, and that is load-bearing rather than fussy. `tokenize` drops
   stopwords, so "This Site" — the portfolio project's own client — tokenises to
   the single term `site`, which is one of the ten measured tokens that must
   never open the gate alone. A name is exhausted by one word only if it WAS one
   word.

   Measured over the shipped corpus this admits exactly six terms —
   domestina, domestina.bg, spetema, mindromeda, paraflow, independent — of
   which the first five already passed the capitalisation test. It adds no
   constant, and the surviving conjuncts in makeGate (own name, surface
   frequency under the shared-name limit) are unchanged.
   ============================================================ */
function soleNames(names) {
  const sole = new Set();
  for (const s of names) {
    /* One authored word, capitalised by the author as a proper noun. */
    if (/\s/.test(s) || !/^\p{Lu}/u.test(s)) continue;
    const terms = tokenize(s);
    if (terms.length === 1) sole.add(terms[0]);
  }
  return sole;
}
  ...
  const entityGate = function entityGate(query) {
    ...
    for (const surface of surfaces) {
      ...
      /* (a) NAMED OUTRIGHT. One term is enough when it is the entity's own
             ...
             Proper-nounhood is read two ways, and a term needs only one of
             them: the prose capitalises it wherever it occurs, OR it is the
             whole of this entity's authored name. The second exists because an
             org can be named with an ordinary word — "Independent" — and see
             soleNames above for why that is not a hole in the first. The other
             two conjuncts are unconditional either way. */
      const namedOutright = matched.some(
        (t) =>
          surface.name.has(t) &&
          sf.get(t) < SHARED_NAME_LIMIT &&
          (isProperName(t) || surface.sole.has(t))
      );
