...
/* ============================================================
   ...
   WHAT THE PORT CHANGED, exhaustively. The ENGINE — the Region class and every
   method on it, the terminator pass, the wall collection, the lattice reads —
   is the source's, line for line, comments included. Around it:

     1. The IIFE became `initAutomata(): { rescan, destroy }`. A React effect
        that mounts a simulation owes the next unmount its teardown, and a
        client-side navigation owes it a re-scan.
     2. Type annotations. No expression changed to satisfy one; where the source
        relied on a property existing (`this.cell` before `build()` runs) the
   ...
   Conway's Life runs in two kinds of region: the side rails of every band, and
   the separator strips. Each region is ONE aria-hidden <canvas> lying over the
   region's own box. The simulation core is typed flat arrays, an age ramp, an
   accent-blue lineage, a toroidal wrap, the breath sine, click-to-seed.

   THE LATTICE IS NOT DECLARED HERE, AND NEITHER IS ITS ORIGIN. The step is
   `--space-6`, resolved once by the graph paper's `background-size` on the
   LATTICE ROOT — `.sheet`, or a `.band` that is not inside one — and read back
   from the computed style below. Restating 24 in this file would let the
   gradient and the canvas disagree about where a cell starts, and they are
   drawn on top of each other.

   The origin is the root's too. Cell (0,0) is the root's top-left corner, not
   the region's, so every region draws its first row and column at MINUS its own
   phase — see `build()`. Taking both numbers from one element is what makes a
   cell boundary in a rail the same line as a cell boundary in the strip below.

   Interactions: hovering a cell lights it accent-blue (this file draws it —
   there is no per-cell element left to carry `:hover`); clicking seeds life, and
   cells born from a click carry an accent lineage that fades toward stone over
   ~10 generations. Each region breathes — cell opacity rides a slow sine
   (~36 s, phase-offset per region) down to near-invisible.
   ============================================================ */

export interface AutomataStats {
  regions: number;
  built: number;
  lattice: number | null;
  offPhaseX: number;
  offPhaseY: number;
  terminators: number;
  cellsPerGeneration: number;
  visibleCells: number;
  maskedByWalls: number;
  offStageCells: number;
  generations: number;
  bitmapMegapixels: number;
  running: boolean;
  reducedMotion: boolean;
}

export interface AutomataHandle {
  /** Re-inject rails, adopt new regions, drop detached ones, re-settle. */
  rescan(): void;
  destroy(): void;
}
...
interface Wall {
  el: Element;
  rect: DOMRect;
  fixedRoot: Element | null;
}

export function initAutomata(): AutomataHandle {
  /* Reduce-motion is re-read on change, exactly as the two inks below are
     re-read on `themechange`. Both are settings the reader can flip while the
     page is open, and a reader who turns motion on mid-visit must not be left
     mid-generation. */
  ...
  /* OFF-STAGE PADDING — one constant, re-derived, used as columns by a rail and
     as rows by a strip. Three is not a compromise between 6 and 0, it is the
     floor the geometry gives you. The off-stage band is one contiguous strip on
     the torus, so the question is the narrowest band that can hold a complete
     moving structure entirely out of sight. A glider's bounding box is 3×3. At
     2 the band cannot contain one without it touching both visible edges at
     once — off-stage stops being a place and becomes a seam. At 3 it is a
     place, 72px of it, and a glider takes ~12 generations (≈3.6s at this tick)
     to cross it and arrive. */
  const HIDDEN = 3;

  /* A torus narrower than 3 makes a cell its own neighbour twice over and Life
     stops being Life. Regions this small are clipped edges, not bugs. */
  const MIN_SIM = 3;
  ...
  /* ALPHA IS PER CELL, fillStyle IS PER DRAW CALL — which is the whole reason
     ...
     built once, and every cell in it is filled under that one fillStyle. */
  const BUCKETS = AGE_STEPS + 1 + BLUE_LIFE; // 9 resident + 10 accent = 19
  const styles = new Array<string>(BUCKETS); // refilled per render, never grown

  /* ---- Walls: content the life has to flow around ----------------------
     Any element whose rect genuinely lies over a region can mark the cells it
     covers permanently dead, so the population goes around the words instead of
     under them. TWO ORIGINS ARE IN PLAY AND ONLY ONE OF THEM IS THE REGION'S.
     The canvas is the region's border box exactly; the LATTICE is the root's,
     and the grid is drawn at minus the region's phase, so turning a rect into
     cell indices does carry a phase term — see it applied in `markWalls`.
     ...
     · A rect is only comparable to a region if the two share a nearest
       `position: fixed` ancestor. That is what keeps the floating bar out of the
       page's rails: the bar lies over the sheet in VIEWPORT space, so which
       cells it covers is a function of scroll, not of layout — a wall that slid
       down the rail as you scrolled would be a wiper.
     ...
     MEASURED on the vanilla index: this masks ZERO cells at 375, 768 and 1280,
     because a `.well` is BESIDE a rail rather than over it. The code stays
     because it is what makes that a measurement rather than an assumption, and
     because `[data-automata-wall]` is how anything that ever does lie over a
     region becomes a wall without touching this file. */
  const WALLS = ".well, .sec__head, [data-automata-wall]";
  const WALL_INSET = 0.25; // fraction of a cell a rect must cover to kill it

  function fixedRootOf(el: Element): Element | null {
  ...
  /* ---- The lattice root, for both of the numbers a region needs ----------
     `.sheet` is the page's; a `.band` that is NOT inside one is its own. The
     `?? el` tail is a degradation rather than a case that exists: a region with
     neither ancestor reads its own box, gets phase 0, and draws the grid it
     drew before any of this. */
  const latticeRootOf = (el: Element): Element => el.closest(".sheet") ?? el.closest(".band") ?? el;
  ...
  /* ---- THE TERMINATOR PASS: the vertical half of the lattice --------------
     ...
     holds, so its last row is a fraction of a cell. The `.term` is the only
     element in a plate whose height is nobody's content, so it is where the
     remainder goes. It has a one-cell base in components.css and a
     `--term-slack` hole; this writes the hole.

     WRITE `--term-slack`, NEVER `height`. The base then stays in the stylesheet,
     `0px` is a real reset, and — the reason it matters beyond tidiness — a
     ...
     · MEASURE AGAINST THE ROOT, NOT AGAINST THE RAIL'S OWN HEIGHT.
     · THE GUARD IS TWO-SIDED AND THAT IS NOT PARANOIA. Layout is subpixel, so a
  ...
  const offLattice = (phase: number, cell: number) => phase > TERM_EPS && phase < cell - TERM_EPS;

  function settleTerminators(scope: ParentNode) {
    const plates: { band: Element; term: HTMLElement; rail: Element }[] = [];
    ...
    for (const { band, term, rail } of plates) {
      /* `closest` matches self, so for a band this resolves to its .sheet if it
         has one and to the band itself if it does not. */
      ...
      /* A zero box is survivable and expected: every region is `display: none`
         on paper. Leaving the slack at its reset 0px is the right answer. */
  ...
  /* Collected ONCE per rebuild pass, not once per region: 21 regions × 9 wells
     ...
     same every time. */
  function collectWalls(root: ParentNode = document): Wall[] {
    const out: Wall[] = [];
  ...
  class Region {
    ...
    /* Assigned by build(), which every path runs before anything reads them. */
    cell!: number;
    w!: number;
    h!: number;
    phaseX!: number;
    ...
    wall!: Uint8Array;
    vis!: Int32Array;
    cellBucket!: Int8Array;
    order!: Int32Array;
    bCount!: Int32Array;
    bStart!: Int32Array;
    bCur!: Int32Array;

    /* The port's, not the source's: the handlers are kept so dispose() can take
       them off a `.strip` that this file did not create and must not delete. */
    private onClick: (e: MouseEvent) => void;
    private onPointerMove: (e: PointerEvent) => void;
    private onPointerLeave: () => void;

    constructor(el: HTMLElement, kind: "rail" | "strip") {
      this.el = el;
      this.kind = kind;
      ...
      /* The region is decoration and so is its canvas. Both say so, because the
         rails are injected here and a strip's attribute is in markup that a
         future page might forget. */
      el.setAttribute("aria-hidden", "true");
      let canvas = el.querySelector<HTMLCanvasElement>("canvas.automata");
      if (!canvas) {
      ...
      this.build();
      this.onClick = (e: MouseEvent) => this.seedAt(e);
      /* Hover is this file's to draw now. There is no per-cell element left, so
         there is nothing for `.sq:hover` to match. */
      this.onPointerMove = (e: PointerEvent) => this.hoverAt(e);
    ...
    /** The port's. Removes what the constructor added, and nothing else. */
    dispose() {
      this.ready = false;
      this.running = false;
      this.el.removeEventListener("click", this.onClick);
      this.el.removeEventListener("pointermove", this.onPointerMove as EventListener);
      this.el.removeEventListener("pointerleave", this.onPointerLeave);
      this.canvas.remove();
    }

    /* CSS owns the box, this owns the bitmap — never the other way round. A zero
       box is survivable and expected: every region is `display: none` on paper. */
    build(walls?: Wall[]) {
      this.ready = false;
      const el = this.el;
      ...
      /* How far this region's corner sits past the last lattice line. Drawing
         starts at MINUS this, so a cell boundary in a rail is the same line as a
         cell boundary in the strip below it — the sheet is one continuous space
         of squares rather than twenty-one adjacent ones. `phaseX` is 0 for every
         region at every width (the columns snap); `phaseY` is what the band
         above ended on, and the terminator drives most of it to 0. */
      ...
      /* The bitmap is device pixels, the box stays authoritative. Clamped at 2
         on purpose: the ink is axis-aligned rectangles on an integer lattice, so
         past 2 there is no edge left to resolve. Re-read on every rebuild, so a
         zoom or a monitor change re-scales rather than blurring. */
      ...
      /* `ceil`, not `floor`: the trailing cell is clipped by the region's
         ...
         whenever the region does not begin on a line. */
      ...
      /* visible index -> sim index, resolved once. It used to be recomputed per
         cell per frame; it is the same table every time. */
      ...
      this.ready = true;
      this.seed();
      /* Under reduce-motion nothing will ever advance this grid, so a fresh seed
         would be frozen ON the random noise it starts from. Settling it here
         rather than once at startup is the fix for a case the old engine missed:
         a resize, or the webfont rebuild that lands a moment after load,
         replaced the settled picture with raw noise and there was no loop left
         to work it out. */
      if (RM) for (let i = 0; i < SETTLE; i++) this.step();
      this.render(this.lastBreath);
    }

    markWalls(box: DOMRect, walls: Wall[]) {
    ...
    seedAt(e: MouseEvent) {
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          const j =
            ((sy + dy + this.simR) % this.simR) * this.simC + ((sx + dx + this.simC) % this.simC);
    ...
    /* ---- Conway, unchanged. Walls are dead cells that stay dead, so they are
       already absent from every neighbour sum: nothing here has to know about
       them except the one line that refuses to let a birth happen. ---- */
    step() {
    ...
    /* ---- Paint ----
       One clearRect, then a counting sort of the living cells into the 19 alpha
       buckets, then one fillStyle and a run of fillRects per bucket that has
       anything in it. Nothing is allocated: every array below was sized with the
       grid.
       ...
       over — the cell sits INSIDE its square rather than on top of it.

       And the leading edges are the ROOT'S, which is what `-phase` buys: the
       square a cell is drawn inside is the same square the gradient drew, so the
       1px inset lands on the same 1px rule it was measured against. */
    render(breath: number) {
      ...
      const { a, age, blue, vis, cellBucket, order, bCount, bStart, bCur, cols, cell, phaseX, phaseY } =
        this;
      const n = vis.length;
      bCount.fill(0);
      ...
      for (let i = 0; i < n; i++) {
        ...
        const bucket =
          blue[si] > 0 ? AGE_STEPS + Math.min(blue[si], BLUE_LIFE) : Math.min(age[si], AGE_STEPS);
        cellBucket[i] = bucket;
        bCount[bucket]++;
      }
      ...
      if (this.hover >= 0 && this.hover < n) {
        ...
        ctx.fillRect(
          (this.hover % cols) * cell + x0,
          ((this.hover / cols) | 0) * cell + y0,
          size,
          size
        );
      }
  ...
  /* ---- Inject rails into every band that has a well ----
     THE RAILS ARE NOT IN THE MARKUP, on either surface. They are two empty divs
     whose only child is an absolutely-positioned canvas this engine owns, so a
     page that never runs it has no orphan decoration to explain. */
  function injectRails(root: ParentNode) {
  ...
  /* ORDER, AND IT IS THE ONE ORDERING IN THIS FILE THAT IS NOT NEGOTIABLE. The
     rails have to exist before the terminators can be measured against them, and
     the terminators have to settle before any canvas is sized — the slack moves
     the rails, so a bitmap built first is a bitmap built for a height that is
     about to change. Nothing is drawn yet at this point: the Region constructors
     below are what build the canvases. */
  ...
  /* ONE SET OF REGIONS. There used to be two — the page's, and a second set
     ...
     separately. The five case studies are pages now, so every region on any page
     belongs to that page's sheet and there is nothing to split. */
  const all: Region[] = [];
  const adopt = (el: Element) =>
    all.push(new Region(el as HTMLElement, el.classList.contains("strip") ? "strip" : "rail"));
  document.querySelectorAll(".rail, .strip").forEach(adopt);
  ...
  function rebuild(regions: Region[], scope: ParentNode) {
  ...
  /* Webfonts change section heights, so they change every plate's remainder too
  ...
  // Both inks belong to the theme. Re-read them and repaint the generation that
  // is already on screen — the life goes on, it just changes colour.
  const onThemeChange = () => {
    readPalette();
  ...
  /* ---- The engine watches its own lattice root ----
     ...
     something changed the page's layout after the last rebuild, and no resize
     event told anyone. Fonts were handled, then a script that injected
     structure, then the scroll lock's scrollbar toggle — and each fix enumerated
     one shifter while the next waited.

     So stop enumerating. The lattice root's own size is the sum of every layout
     fact the terminator pass depends on: if ANY of them moves, the sheet's
     height moves, and this observer funnels it into the same debounced rebuild a
     resize uses. It converges: a rebuild writes the same slack values against
     unchanged layout, the root's size stops changing, the observer goes quiet. */
  const ro = typeof ResizeObserver === "function" ? new ResizeObserver(settle) : null;
  const observeRoot = () => {
    if (!ro) return;
    ro.disconnect();
  ...
  /* ---- Run, or stand still ----
     ...
     Flipping the OS setting mid-visit starts or stops the loop in place. The
     generation on screen is kept either way: turning motion back on resumes the
     life that was already there rather than reseeding it — and turning it ON
     freezes what is on screen rather than re-settling, because the reader is
     looking at that. */
  let rafId = 0;
  ...
  RMQ.addEventListener("change", onMotionChange);

  /* ---- What this costs, stated by the thing that spends it ----
     The div model's numbers had to be reverse-engineered from layout every time
     anyone wanted them. The engine can just say. Returns live totals for the
     four figures the skeleton spec is gated on — and, on this surface, the
     numbers the browser parity sweep compares against the vanilla page. */
  window.automataStats = (): AutomataStats => {
    const live = all.filter((r) => r.ready);
    const sum = (f: (r: Region) => number) => live.reduce((t, r) => t + f(r), 0);
    return {
      regions: all.length,
      built: live.length,
      ...
      /* TWO-SIDED, for the same reason the terminator's own guard is: a region
         ...
         as off-lattice would report a rounding artefact as a defect. */
  ...
  /* ---- The port's two exports ----
     `rescan` exists because this surface can change its document without a page
     load. The vanilla site cannot: every link there is a navigation, so the
     engine only ever had to be right once. Here a client-side transition
     replaces the bands under a running simulation, so regions whose element has
     left the document are disposed, new ones are adopted, and the whole set is
     rebuilt through the same single path everything else uses. */
  function rescan() {
    if (destroyed) return;
    injectRails(document);
    for (let i = all.length - 1; i >= 0; i--) {
      if (all[i].el.isConnected) continue;
      io.unobserve(all[i].el);
      all[i].dispose();
      all.splice(i, 1);
    }
    document.querySelectorAll(".rail, .strip").forEach((el) => {
      if (all.some((r) => r.el === el)) return;
      adopt(el);
      io.observe(el);
    });
    observeRoot();
    rebuild(all, document);
  }
  ...
  function destroy() {
    ...
    RMQ.removeEventListener("change", onMotionChange);
    io.disconnect();
    ro?.disconnect();
    all.forEach((r) => r.dispose());
    all.length = 0;
    /* The rails were injected by this file, so they leave with it; the strips
       and the terminators are the page's, so they are only reset. A terminator
       left holding slack after the engine has gone would be decoration nothing
       owns — and `0px` is the base the stylesheet already draws. */
    document.querySelectorAll(".rail").forEach((el) => el.remove());
    document
      .querySelectorAll<HTMLElement>(".term")
      .forEach((el) => el.style.setProperty("--term-slack", "0px"));
    delete window.automataStats;
  }
